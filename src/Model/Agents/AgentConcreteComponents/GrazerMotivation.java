    ...
    public AgentDecision run(AgentVision tile, Attributes attributes, Scores scores) {
        if (!tile.isOccupied()) { // Grazer motivation does not motivate agent to travel to occupied tiles
            if (tile.getFoodLevel() > 0) {
                // Tile is not occupied and has food, set decision to GRAZE and score to the tiles food level multiplied by how much energy its missing
                return new AgentDecision(tile.getLocation(), AgentAction.GRAZE, tile.getFoodLevel() * (attributes.getEnergyCapacity() - scores.getHunger()));
            }
