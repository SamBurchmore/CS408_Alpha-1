...
//        activeAgents.addAgent(new BasicAgent(new Location(-1, -1),                      new BasicAttributes(1, "Cyan", 7, Color.cyan,                           1, 1, 10, 2, 48, 8, 3, 9), (ArrayList<Motivation>) grazerMotivations.clone()), 7);
        for (int i = 0; i < 8; i++) {
            activeAgents.addAgent(new BasicAgent(new Location(-1, -1),                      new BasicAttributes(1, agentNames[i], i, agentColors[i], ColorModel.RANDOM, 1,1, 5, 4), (ArrayList<Motivation>) omniMotivations.clone()), i);
        }
